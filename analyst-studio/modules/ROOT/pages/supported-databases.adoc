...
We are committed to improving the experience of users connecting {brand} to datasources of these types, and will fix issues based on severity and demand.
...
NOTE: If the database supports multiple Drivers, use the xref:managing-database-connections.adoc#update-preferred-jdbc-driver[Manage Connections] page to update to your prefered driver.
...
When creating an Athena connection, we recommend link:https://docs.aws.amazon.com/athena/latest/ug/setting-up.html[creating a user in AWS] that only has access to the data you want to query through Athena.
...
. Navigate to the link:https://console.aws.amazon.com/redshift/[Redshift Management Console].
...
For additional questions about getting your DataVault credentials, please free to contact link:mailto:support@tenjin.io[support@tenjin.io].
...
There is a link:https://github.com/apache/druid/pull/10880[server-side fix] that can be applied.
...
| {brand} currently does not support changing the Druid broker's link:https://druid.apache.org/docs/latest/querying/sql.html#connection-context[setting for the connection] via JDBC properties.

| Avatica 1.17.0
| Only Druid SQL support
| When connecting to Druid via JDBC, link:https://druid.apache.org/docs/0.20.1/querying/sql.html[only Druid SQL syntax is supported.] Native (json-type) query syntax will result in errors.

| Avatica 1.17.0
| Druid SQL limitations
| Not all Druid features are supported via Druid SQL and JDBC connectivity.
Please see the link:https://druid.apache.org/docs/0.20.1/querying/sql.html#unsupported-features[Druid docs.]
...
. *Host:* the route of the Druid cluster link:https://druid.apache.org/docs/latest/design/broker.html[Broker], as specified in the link:https://druid.apache.org/docs/latest/configuration/index.html#broker[broker/runtime.properties] file.
. *Port:* the port of the link:https://druid.apache.org/docs/latest/design/broker.html[Broker].
...
*Please note the following link:https://druid.apache.org/docs/latest/querying/sql.html#connection-stickiness[limitation] of broker connection stickiness over JDBC:*
...
The Hive JDBC driver cannot trigger the cluster to automatically restart, so you may want to adjust the timeout or disable automatic termination per Databricks' link:https://docs.databricks.com/user-guide/clusters/terminate.html#configuration[documentation].
...
This permission can be added via several link:https://cloud.google.com/bigquery/docs/access-control[roles].
...
. Navigate to the link:https://console.developers.google.com[Google Developers Console].
...
When connecting to BigQuery, you will choose either link:https://cloud.google.com/bigquery/docs/reference/legacy-sql/[legacy SQL] or link:https://cloud.google.com/bigquery/docs/introduction-sql[standard SQL] as the link:https://cloud.google.com/bigquery/docs/reference/standard-sql/migrating-from-legacy-sql[default SQL dialect] for that connection.
...
Learn more about link:https://cloud.google.com/storage/docs/authentication#service_accounts[BigQuery service accounts and P12 keys].
. Click *Save Changes*.
+
...
+
Navigate to the link:https://console.developers.google.com/[Google Developer Console] and select your project from the project dropdown menu.
...
+
Go to the link:https://bigquery.cloud.google.com/[BigQuery Web UI].
 ...
 ** To get your service account email address, go to your link:https://console.cloud.google.com/[Google Cloud Console], switch to your Project, and click on Service Accounts.
...
For known limitations, see link:https://cloud.google.com/looker/docs/sql-interface#lookml_limitations[Looker's documentation].
...
. Log into your link:https://manage.windowsazure.com/[Azure account] and select *SQL Databases* from the left navigation.
...
More details in link:https://stackoverflow.com/questions/26515700/mysql-jdbc-driver-5-1-33-time-zone-issue[this StackOverflow thread].
|===
...
. Navigate to your link:https://cloud.oracle.com/db/adb[Oracle Cloud Autonomous Databases page] and click on your instance. +
...
NOTE: mTLS requirement must be relaxed link:https://docs.oracle.com/en/cloud/paas/autonomous-database/adbsa/support-tls-mtls-authentication.html#GUID-6A34B30A-3692-4D1F-8458-FD8F32736199[to allow for TLS] (default for ADB is to require mTLS).
...
If you need your Heap credentials, please email link:mailto:support@heapanalytics.com[support@heapanalytics.com] for help.
...
Please follow the link:https://docs.snowflake.com/en/user-guide/key-pair-auth#configuring-key-pair-authentication[Snowflake documentation] for configuring key pair authentication.
...
Please follow the steps outlined link:https://docs.treasuredata.com/display/public/INT/Mode+Analytics+Export+Integration[in the Treasure Data documentation] to connect Treasure Data to {brand}.

If you need help finding your database credentials, please contact your Treasure Data account manager at link:mailto:info@treasure-data.com[info@treasure-data.com].
...
. Navigate to your link:https://console.aws.amazon.com/rds/home#dbinstances:[RDS Instance Console].
...
Your instance is in a VPC if there's a entry labeled VPC.
