...
Inspired by the layered link:https://towardsdatascience.com/a-comprehensive-guide-to-the-grammar-of-graphics-for-effective-visualization-of-multi-dimensional-1f92b4ed4149[Grammar of Graphics], Visual Explorer provides an environment where you can create and configure your visualizations based on a ruleset--a grammar, if you will.
...
If you have a multi-dimensional dataset, adding a *Dimension* to your Columns or Rows is a powerful starting point to begin slicing your data.
...
Check out our link:https://mode.com/visual-explorer-guides/[Visual Explorer field guide] for a walkthrough on how to create visualizations in the Visual Explorer.
...
Measures can also be added to the Color channel by dragging and dropping or using the type ahead search.
...
Adding a continuous numeric Measure to the *Size* channel by dragging and dropping or using the type ahead search will reflect the value of that measure in the width of every mark in that particular layer.
...
Adding a field to the *Text* channel by dragging and dropping or using the type ahead search will display text on your visualization.
...
As you'll begin to see in some of our link:https://mode.com/visual-explorer-guides/[example configurations], `Measure Names` and `Measure Values` are integral pieces to build certain visualizations.
...
. Drag-and-drop or use the type ahead search to add `Measure Values` to either Rows or Columns--whichever axis you want your measures to be on.
+
...
. Drag-and-drop or use the type ahead search to add at least two measures you want to graph on either Rows or Columns.
...
To create a shared axis, add at least one continuous field to columns or rows by dragging and dropping or by using the type ahead search.
...
image:shared-axis/1.1.png[Shared Axis 1.1, 1.2]
...
image:shared-axis/1.3.png[Shared Axis 1.3]
...
You can also click directly on the Format tab in the righthand panel of the visualization builder (fig 2.2a).
...
Check out our link:https://modeanalytics.com/nascif_abousalh_neto/reports/d3eeb27acd63[Example Report] for inspiration, and duplicate it to your own Workspace to see how the visualizations are built.
...
. Add your field (text, date, and numeric fields) to the filter section by dragging and dropping or by using the type ahead search.
...
*Combined legends* facilitates comparisons across categories that share units, and can help you to understand the relative distribution of specific field data within a larger set of fields.
...
Text settings enables you to quickly label important data points in your visualization, and provide at-a-glance insight for complex charts.
...
You can drag fields within and between the categories to control how the table calculation will be applies.
...
This will color the values in your pivot table based on the the cell's value according to that field.
