ifndef::pendo-links[]
May 2024 [label label-dep]#9.12.0.cl#
endif::[]
ifdef::pendo-links[]
...
Administrators can enable version control of Liveboards and Answers with the new GIT integration feature. You enable version control for your ThoughtSpot Org by connecting it to a Github repository. Once enabled for the Org, you can then enable version control individually for any Liveboard or Answer. Each time a user saves changes, a new version of the Liveboard or Answer is created and stored in your Github repository. Users can easily browse and restore an older version from the Liveboard or Answer itself.
