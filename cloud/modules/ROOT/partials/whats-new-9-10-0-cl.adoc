...
// Naomi -- scal-173345, scal-89341. GA! documentation jira: scal-201032
// PM: Vikas
...
// Naomi -- SCAL-127727, SCAL-164086. documentation jira scal-201033. add a gif. ask if the filter appears in your email alerts, so users know the data they're getting is filtered. (they don't)
...
Configure tooltips by selecting the *Edit chart configuration* icon image:icon-gear-10px.png[chart customization icon] and selecting *Settings*. Under *Tooltip* click *Edit tooltip*. By default, fields visualized in charts are included in the tooltip. You can remove, add, or rearrange fields.
...
image::models_whats_new.png[Models]
...
[discrete]
=== Custom sorting
Custom sorting is now available to all users and is on by default. ThoughtSpot introduces custom sorting of attributes to sort most chart types in a way that helps users to consume charts more efficiently. The following chart types are supported:
...
You can now sort on any attribute in a chart, using the custom sort column. Enter the attributes in order to set up a custom sort order.
