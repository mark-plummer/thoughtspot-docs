...
ThoughtSpot Cloud 10.6.0.cl is now available.
...
You can now edit tags for tables, Worksheets, views, and SQL views from the Data workspace home page. Refer to xref:tags.adoc#data-workspace-create[Use tags to organize].
...
This release introduces the ThoughtSpot Embedded Edition license, Visual Embed SDK version 1.4.0, and new REST APIs for programmatic management of data connections, metadata objects, user accounts, user groups and privileges, and object sharing permissions.
...
* *TML for tables with row-level security:* ThoughtSpot now supports the migration and editing of tables with xref:security-rls-concept.adoc[row-level security (RLS)] using xref:tml-tables.adoc#syntax-tables[TML].
...
Refer to xref:consumption-pricing.adoc#credits[Consumption-based pricing] for more information.
[#admin-privileges]
...
See xref:tml-worksheets.adoc#syntax-worksheets[Syntax of the Worksheet TML file].
...
The *Download as CSV*, *Download as XLSX*, and *Download as PDF* actions are available for table visualizations.
...
Refer to xref:tml-tables.adoc#syntax-tables[ThoughtSpot Modeling Language] and specify the `connection` information when adding tables or table columns to an Embrace connection.
[#query-dashboard]
...
[#developer-privilege]
Developer privilege::
You can now grant groups the xref:privileges-end-user.adoc[Developer privilege] from the Admin Console, allowing users in those groups to access and use the Developer Portal.
...
The portal also allows authorized users to rebrand the embedded content, xref:developer-user.adoc[customize styles], and xref:developer-user.adoc[Custom Actions] to the embedded instance. For more information, see ThoughtSpot Developer Portal.
...
Refer to xref:tml-tables.adoc#syntax-tables[ThoughtSpot Modeling Language]. +
