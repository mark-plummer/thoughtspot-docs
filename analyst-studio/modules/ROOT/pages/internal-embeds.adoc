...
NOTE: Generally, you must be logged in to {brand} and have access to the underlying report in order to view an Internal Embed. However, the above embedded report is built on xref:managing-database-connections.adoc#mode-public-warehouse[{brand}'s Public Warehouse] and is therefore viewable by anyone on the internet. Learn more about <<embedding-reports-built-using-the-mode-public-warehouse,embedding reports built on {brand}'s Public Warehouse>>.
...
Only users that are signed in to {brand} are able view internal embeds.
* {blank}
