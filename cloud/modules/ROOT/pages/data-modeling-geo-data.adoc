...
After loading the data, ThoughtSpot sets the geo config data under auto select, which automatically detects the type of geo data in the column and classifies it. You can also designate the column as a geographical data type when you xref:data-modeling-edit.adoc#[edit the system-wide data model].
