...
{brand} offers a number of options for building visualizations, including the ability to take your analysis even further using Python / R or using any type of custom visualization using JavaScript (for example, link:https://d3js.org/[D3]).
...
The require exactly one field on the X-axis and one field on the Y axis with optional fields for color and size
...
Note that the the default sort order is ascending by the field you have in the *Trend by.
...
This can be accomplished by using the drag-and-drop method or by using the type-ahead search option.
...
image::present-your-data/format-table.png[toolbar2]
...
The number of rows shown per page can be customized by adding any number between 1 to 1000 in the input box located in the bottom right corner.
...
A link:https://en.wikipedia.org/wiki/Pivot_table[pivot table] aggregates and summarizes (for example, using sum, average, count, etc.) query results and presents them in a data table.
...
This can be done by dragging and dropping fields or by using the type-ahead search option.
...
You can add xref:visual-explorer.adoc#filters[filters] to pivot tables by adding fields, including xref:cal-fields.adoc#overview[calculated fields], into the filters drop zone by dragging and dropping or by using the type ahead search.
...
NOTE: We now no longer actively support display tables and we are in the process of sunsetting them at some point in the future.
...
* *Sort:* Click the header of the column you'd like to sort the table by and click the *drop down* image:action-movedown-hover.svg[drop down] button to choose how you want the table sorted.
...
All Quick Charts except Tables and Big Values can be be converted to xref:visual-explorer.adoc[Visual Explorer] to access advanced chart options.
...
Every report contains a built-in xref:notebook.adoc[Notebook] environment where you can take the results of one or more queries, further analyze them in Python or R, and visualize the results using one of the many xref:notebook.adoc.adoc#available-libraries[built-in libraries].
...
To help you get started, {brand}'s community maintains the link:https://github.com/mode/alamode[Alamode] JavaScript library that makes creating link:https://d3js.org/[D3-powered] visualizations a snap.

Check out link:https://mode.com/example-gallery/[{brand}'s Gallery] for inspiration, sample reports, and more information about how to create advanced visualizations in {brand}.
...
The type-ahead search in the dropzones can also be used to add the new fields.
